...
function normalizeScene(raw, dreamText) {
  ...
  // a hole with people at the bottom has to be wide enough to be looked into: 1.1 m across and 4 m
  // deep needs an eighty-five degree camera, which is a plan view of everyone standing at the rim
  ...
  { const roomOf = q => actors.find(rm => (rm.kind === 'room' || rm.kind === 'corridor')
    ...
    for (const b of beats) {
      ...
      // ...or whether the sentence merely says they talked: "I stop to talk with him" gives
      // nobody a line and is still two people facing each other
      const chatty = /\b(talk|talks|talked|talking|said|says|say|saying|ask|asks|asked|tell|tells|told|speak|speaks|spoke|answer|answers|answered|yell|yells|yelled|shout|shouts|shouted)\b/i.test(b.text || '');
      { const talkers = b.actions.filter(x => x.actor && (x.say || FACE_STATE.has(x.state) || chatty) && (actors.find(q => q.id === x.actor) || {}).kind === 'person');
  ...
  // nothing but the sky belongs above a room's ceiling: an orb the director puts at y 3 in a 3.5 m
  // room, at size 1.6, has its bottom above the lamp and is in no shot of the room at all
  for (const rm of actors) { if (rm.kind !== 'room' && rm.kind !== 'corridor') continue;
    const hw = (rm.detail.width || 8) * rm.size / 2, hd = (rm.detail.depth || (rm.kind === 'corridor' ? 30 : 8)) * rm.size / 2;
    const ceil = (rm.detail.height || 3) * rm.size;
    for (const a of actors) { if (a === rm || ['moon', 'sun', 'star', 'cloud', 'room', 'corridor', 'building', 'house'].includes(a.kind)) continue;
      if (Math.abs(a.pos[0] - rm.pos[0]) > hw || Math.abs(a.pos[2] - rm.pos[2]) > hd) continue;
      const top = a.pos[1] + Math.max(0.4, a.size * 0.9);
      if (top > ceil - 0.35) a.pos[1] = Math.max(0.2, ceil - 0.35 - Math.max(0.4, a.size * 0.9)); } }
  // a beat that says we are inside something puts us inside it: a cave eleven metres past the two
  // people it is meant to be around reads as a lit hut they are walking past
  for (const b of beats) {
  ...
  for (const b of beats) {
    ...
    // all of it or none of it: half a push leaves the same complaint for the next pass to find,
    // and a scene that normalises differently the second time is a scene the director chat breaks
    if (back > 0.05 && back <= 2) wants.set(tid, Math.max(wants.get(tid) || 0, back)); }
  ...
  for (const b of beats) {
    ...
    for (const x of b.actions) { if (!x.actor || x.actor === tid) continue;
      ...
      // and only for somebody actually standing in the shot. A forty-eight-degree cone caught two
      // people Claude had already placed one to each side, two metres apart, and threw them out to
      // seven -- which is a wide of the party where the report has a conversation.
      const along = dx * fx + dz * fz, perp = Math.abs(dx * fz - dz * fx);
      const camD = Math.max(2, b.camera.distance || 5);
      if (along <= 0.2 || along > camD || perp > 0.9) continue;
