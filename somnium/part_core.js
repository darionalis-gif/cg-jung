...
6. Stage transitions of place as a beat with a "blackout" effect at 0 plus "world" changes and moves, or by having the old scenery vanish and the new appear. Whatever the beat is about must be on stage for most of it: give an "appear" an "at" of 0.15 or less. Actors that are not yet in the story start hidden and get "appear"; actors that leave get "vanish" (both happen within about a quarter of the beat unless you give "for"). Hidden actors still need a pos. A crowd's yaw turns its members: point a crowd at whatever it is watching, crowded around or complaining at, not at the scenery behind it. A moon or sun is a disc at exactly its pos: put it 60-150 m away, only 15-40 m up so it hangs low over the horizon, and on the side the camera of the beats that show it will be facing (for a follow camera that is behind the actor, so a moon the dreamer walks toward goes in front of them).
...
function skyPos(p, inRoom) { let [x, y, z] = p; if (inRoom) return [x, y, z]; let h = Math.hypot(x, z); if (h < 25) { const k = 25 / Math.max(0.001, h); x *= k; z *= k; h = 25; } if (h > 200) { const k = 200 / h; x *= k; z *= k; h = 200; } return [x, clamp(y, 8, h * 0.13), z]; }
