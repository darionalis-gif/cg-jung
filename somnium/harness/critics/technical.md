...
3. Motion and animation quality — the owner has asked for "proper quality of animation", so judge this strictly: do walk and run cycles look like walking and running (knees, arms, weight, no sliding feet), do poses blend rather than snap, do figures turn smoothly, do heads face whoever they talk to, does the camera move smoothly with no jumps except scripted cuts, do vehicles, animals, fire, water and weather move convincingly? Use the play-*.png sequences (consecutive frames while playing). Does something actually move in every beat where the script says something moves (`motion` near 0 in a beat with a `move` or a non-idle `state` is a defect)? Do walk cycles, flying, falling, vehicles, fire, water, weather read as such in the play frames?
