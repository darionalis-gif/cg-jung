...
const Stage = {
  ...
  evalActors(t) {
    ...
    for (const b of S.beats) { if (t < b.start) break; for (const x of b.actions) { if (!x.actor) continue; const st = out.get(x.actor); if (!st) continue; const a0 = b.start + x.at * b.dur, a1 = Math.min(b.start + b.dur, a0 + x.for * b.dur); const p = a1 > a0 ? clamp((t - a0) / (a1 - a0), 0, 1) : (t >= a0 ? 1 : 0); if (t < a0) continue; const e = easeInOut(p);
      ...
      if (b.actions.some(x => x.effect === 'blackout')) {
        for (const [id, st] of out) { if (b.actions.some(x => x.actor === id && x.state)) continue;
          if (['kneel', 'sit', 'lie', 'fold', 'grieve', 'yell', 'shake', 'pockets', 'wave', 'throw', 'push'].includes(st.state)) st.state = 'idle'; } }
    }
  ...
  evaluate(dt, snap) {
    ...
    const bi = this.beatAt(t); if (bi !== this.lastBeat) { const prev = this.lastBeat; this.lastBeat = bi; if (this.onBeat) this.onBeat(bi); // a beat is a cut. Nothing set snap at a boundary during playback, so the lens flew from the
      ...
      this._unblockDir = null; this._lastAim = null; this._lastCamOff = null; this._rush = 0;
      this.reaimSky = true; }
  ...
  updateCamera(beat, states, dt, snap) {
    ...
    if (!snap && !authored && this.shotHold && this.shotHold.beat === this.lastBeat) {
      ...
      if (tick !== h.tick) { const was = h.tick; h.tick = tick;
        if (h.weak || (arriveTick >= 0 && was < arriveTick && tick >= arriveTick)) this.shotHold = null;
  ...
  roomAround(p) {
    ...
    if (!this._rooms) { this._rooms = []; for (const rec of this.actors.values()) { if (!['room', 'corridor', 'cave', 'tent'].includes(rec.a.kind)) continue; const box = new THREE.Box3().setFromObject(rec.g); this._rooms.push({ rec, box }); } }
