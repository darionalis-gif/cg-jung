...
const App = {
  ...
  async direct(request) {
    ...
    try { let out = null, sceneRaw = null;
      ...
      if (!sceneRaw) throw { code: 'bad_reply', message: 'no scene in the reply' }; const before = d.scene; this.undo.push(d.scene); const t = Stage.time; d.scene = normalizeScene(sceneRaw, d.text); d.title = d.scene.title; const reply = typeof out.reply === 'string' && out.reply.trim() ? out.reply.trim() : 'Changed.'; Stage.load(d.scene); const note = this.visibilityNote(before, d.scene); d.chat.push({ role: 'director', text: reply + (note ? '\n\n' + note : '') }); Stage.setTime(Math.min(t, d.scene.total)); Stage.playing = true; $('#stageTitle').textContent = d.title; this.renderScript(); this.renderChat(); this.renderLib(); this.save(); return d; }
  ...
  // The director cannot see the stage. Three rounds running it told the dreamer a moon was in
  // frame in every beat while the disc sat behind a mountain, because nothing ever checked. So the
  // stage checks, and says what is actually there in the director's own reply.
  visibilityNote(before, after) {
    try {
      const key = a => JSON.stringify([a.pos, a.size, a.color, a.kind, !!a.hidden]);
      const was = new Map((before.actors || []).map(a => [a.id, key(a)]));
      const ids = (after.actors || []).filter(a => was.get(a.id) !== key(a)).map(a => a.id);
      if (!ids.length || ids.length > 3 || !after.beats.length) return '';
      const t0 = Stage.time, seen = new Map(ids.map(i => [i, []]));
      let t = 0;
      after.beats.forEach((b, i) => { Stage.setTime(t + b.dur * 0.5); for (const id of ids) if (Stage.seenAt(id)) seen.get(id).push(i + 1); t += b.dur; });
      Stage.setTime(t0);
      const n = after.beats.length, parts = [];
      for (const id of ids) { const a = after.actors.find(q => q.id === id); const L = (a && a.label) || id; const bs = seen.get(id);
        parts.push(bs.length === n ? L + ' is in frame in all ' + n + ' beats'
          : bs.length ? L + ' is in frame in ' + (bs.length === 1 ? 'beat ' : 'beats ') + bs.join(', ') + ' of ' + n
          : L + ' does not reach the frame in any beat'); }
      return 'Measured on the stage: ' + parts.join('; ') + '.';
    } catch (e) { return ''; }
  },
