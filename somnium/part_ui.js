...
const App = {
  ...
  editPrompt(d, request) { const hist = d.chat.slice(-8).map(m => (m.role === 'user' ? 'DREAMER: ' : 'DIRECTOR: ') + m.text).join('\n'); return DSL_DOC + '\n\nTHE DREAM REPORT:\n<<<\n' + d.text + '\n>>>\n\nTHE CURRENT STAGE SCRIPT (JSON):\n' + JSON.stringify(this.exportScene(d.scene)) + (hist ? '\n\nCONVERSATION SO FAR:\n' + hist : '') + '\n\nTHE DREAMER NOW ASKS:\n' + request + '\n\nRevise the stage script to honour the request while keeping everything not asked about the same (same ids, same beats and text unless the request concerns them). Answer with one JSON object and nothing else: {"reply": "one or two sentences to the dreamer, in plain words, on what changed", "scene": <the complete revised stage script>}.'; },
  exportScene(s) { return { title: s.title, mood: s.mood, world: s.world, actors: s.actors, beats: s.beats.map(b => ({ dur: b.dur, text: b.text, camera: b.camera, actions: b.actions })) }; },
