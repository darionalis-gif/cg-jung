...
B.mountain = a => { const g = new THREE.Group(); const rnd = seeded(9);
  ...
  const rockC = lum > 0.6 ? shade(a.color, 0.58) : a.color; const m = mat(rockC, { flat: true }); for (let i = 0; i < 3; i++) { const h = 40 + rnd() * 40, r = h * 0.9; const c = mesh(G.cone(r, h, 7), m, (i - 1) * r * 1.1, h / 2, (rnd() - 0.5) * r); g.add(c); const snow = mesh(G.cone(r * 0.3, h * 0.3, 7), mat(a.detail.second || (lum > 0.6 ? a.color : '#f0f4fa'), { flat: true }), c.position.x, h - h * 0.15, c.position.z); g.add(snow); } g.userData.height = 80; g.userData.big = true; return g; };
