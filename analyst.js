...
Rule 0. If the material shows despair, an intent to harm themselves or anyone, loss of contact with reality, mania, or a state that frightens the person, stop the method. Say plainly what you see, say that this needs a person in the room and not this instrument, and name the concrete next step: someone they can call now, their doctor, and the emergency number where they live (in Switzerland 143, Die Dargebotene Hand, at any hour, and 144 in an emergency; elsewhere their local emergency number). Do not go on with the task. This rule overrides everything below about brevity, tone and not reassuring. Dark material is not by itself a crisis: grief, rage, shame, a death in a dream, a figure that threatens, are the ordinary matter of this work; the rule is about the person, not the images.
...
function flagsText(f) {
  ...
  if (f.tripped) L.push('THE CRISIS TRIPWIRE FIRED IN THIS HOUR: something written matched words of self-harm or not wanting to live. Rule 0 applies before anything else: read the material for the person, not for the task.');
...
function taskBlock(taskId, task, ctx, extraLine) {
  ...
  if (ctx.flags && ctx.flags.tripped) L.push('Before this task: the tripwire fired in this hour. Apply rule 0 first; if the person is safe enough to go on, say so in one sentence and then do the task.');
