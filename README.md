...
| II | Association | Jung's word-association experiment: 24 stimulus words from his list of 100 (English or German), answered by speaking (Web Speech API, stopwatch to the first sound) or typing (stopwatch to the first keystroke), then a reproduction test in which misspellings and slips do not count as changes (Damerau-Levenshtein tolerance scaled by word length). Complex indicators are computed the way Jung read a protocol: prolonged reaction, no reaction, failed reproduction, several words, echo of the stimulus, perseveration. |
